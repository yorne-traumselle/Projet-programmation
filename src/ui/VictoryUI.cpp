...
VictoryUI::VictoryUI(TTF_Font* font):
	m_font(font), m_textColor{255, 0, 0, 255}, m_buttonColor{50, 50, 50, 255}, m_buttonTextColor{255, 255, 255, 255},
	m_victoryPosition(Vector2<float>(400, 200)), m_buttonPosition(Vector2<float>(100, 40)), m_buttonSize(200, 30),  m_message("Victory !"), m_buttonLabel("Retry !"){
}

void VictoryUI::setVictoryPosition(const Vector2<float>& pos){
	m_victoryPosition = pos;
}

void VictoryUI::setButtonPosition(const Vector2<float>& pos){
	m_buttonPosition = pos;
}

void VictoryUI::setButtonSize(const Vector2<float>& size){
	m_buttonSize = size;
}

void VictoryUI::render() {
    Renderer* renderer = Renderer::getInstance();

    // Ici, on affiche le texte "Victory"
    int textW, textH;
    TTF_SizeText(m_font, m_message.c_str(), &textW, &textH);
    Vector2<float> centeredText = m_victoryPosition - Vector2<float>(textW / 2, textH / 2);
...
bool VictoryUI::handleClick(int mouseX, int mouseY) {
